...
let fkon = { key: { fromMe: false, participant: `${m.sender.split`@`[0]}@s.whatsapp.net`, ...(m.chat ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { contactMessage: { displayName: `${name}`, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;a,;;;\nFN:${name}\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}}
  let nyaww = 'https://telegra.ph/file/a20c924a5f7e7953607aa.jpg'
  let fotodaftar = 'https://telegra.ph/file/a20c924a5f7e7953607aa.jpg'
let msg = {
        rowner: 'Fitur Ini Khusus Developerku',
        owner: 'Fitur Ini Khusus Ownerku',
        mods: 'Fitur Ini Khusus Moderator',
        premium: 'Fitur Ini Khusus Premium User',
        group: 'Fitur Ini Hanya Bisa Digunakan Di Grup',       botAdmin: 'Jadikan bot Sebagai Admin Terlebih Dahulu Agar Bisa Menggunakan Fitur Ini',
        restrict: 'Restict Belum Di Nyalakan Untuk Chat Ini'}[type]
  if (msg) return conn.sendButton(m.chat, msg, wm, nyaww, [['Maaf',`.maaf`]], fkon)
  let daftar = {
  unreg: 'Ketik .daftar namakamu.umurkamu\nContoh : .daftar Esteh.10'}[type]
  if (daftar) return conn.sendMessage(m.chat, { image : { url : fotodaftar }, caption : daftar }, m)
  }
function ucapan() {
  const time = moment.tz('Asia/Jakarta').format('HH')
  let res = "Sudah Dini Hari Kok Belum Tidur rek nganu kah? "
  if (time >= 4) {
    res = "Selamat Pagi cvk"
  }
  if (time >= 10) {
    res = "Selamat Siang ngap"
  }
  if (time >= 15) {
    res = "Selamat Sore rek"
  }
  if (time >= 18) {
    res = "Selamat Malam lur"
  }
  return res
}
